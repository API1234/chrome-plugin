...
### 🎨 界面特色
- **独立词汇表页面**：访问 [https://llm-api-xi.vercel.app/board](https://llm-api-xi.vercel.app/board) 管理词汇
- **多标签页管理**：支持"全部"、"单词表"、"今日待复习"、"历史待复习"四个标签
- **现代化 UI**：美观的卡片式设计，支持主题切换
- **响应式布局**：适配不同屏幕尺寸
...
#### 方法二：手动添加
1. 点击扩展图标，选择"词汇表"按钮打开词汇表页面（或直接访问 [https://llm-api-xi.vercel.app/board](https://llm-api-xi.vercel.app/board)）
2. 点击"添加单词"按钮
3. 输入单词，系统会自动：
   - 规范化单词（转小写、复数转单数）
   - 获取音标、词性、释义
...
**注意**：词汇表页面已独立部署到 [https://llm-api-xi.vercel.app/board](https://llm-api-xi.vercel.app/board)，插件仅负责收集和保存词汇数据。
...
**注意**：本项目为个人学习项目，功能可能随时更新。使用过程中如遇到问题，请检查浏览器控制台的错误信息。
